...
# 9/21/18
# NCSU Libraries 
# Purpose: Solutions to the exercise file for the Beginning R workshop
...
activeBeaver["diffMean"] = activeBeaver$temp - meanTemp

# 26. Practice. Remove the variable day from the activeBeaver dataset. 

activeBeaver$day = NULL
...
# Then reset par(mfrow) back to single plot layout.
...
airbnb_highest <- subset(airbnb, Rating > 95)

write.csv(airbnb_highest, file="airbnb_highest_ratings.csv")
...
# 49. Practice. Calculate the correlation between Beds and Price.
cor(airbnb$Beds, airbnb$Price)

# 50. Practice. Build a linear model using the Price (response) and Beds (term) variables. Name it linearBP.
linearBP <- lm(Price ~ Beds, data=airbnb)
